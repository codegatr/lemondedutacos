...
<head>
  ...
  <title>Le Monde Du Tacos – Tacos Menüsü</title>
  ...
  <style>
    ...
    *{ box-sizing:border-box; }
    ...
    .tabs{
      ...
      overflow-x:auto; padding:0 216px;
  ...
  <main class="page">
    <section class="menu-shell">
      <div class="menu-head">
        ...
        <div class="tabs" role="tablist" aria-label="Tacos kategorileri">
          <button class="tab-btn active" role="tab" aria-selected="true" data-tab="secilmis">Seçilmiş Lezzetler</button>
          <button class="tab-btn" role="tab" aria-selected="false" data-tab="imzali">İmzalı Lezzetler</button>
          <button class="tab-btn" role="tab" aria-selected="false" data-tab="gurme">Gurme Lezzetler</button>
        </div>
      </div>

      <section class="tab-panel active" id="tab-secilmis">
        <h1 class="section-title">Restoranın en çok tercih edilen ürünleri</h1>
        <div class="menu-list">
          
<?php
$cat_id_for_secilmis = null;
foreach ($cats as $c) { if ($c["code"] === "secilmis") { $cat_id_for_secilmis = $c["id"]; break; } }
if ($cat_id_for_secilmis && !empty($items_by_cat[$cat_id_for_secilmis])) {
    render_menu_items($items_by_cat[$cat_id_for_secilmis]);
}
?>
      ...
      <section class="tab-panel" id="tab-imzali">
        <h1 class="section-title">İmzalı Lezzetler</h1>
        <div class="menu-list">
          
<?php
$cat_id_for_imzali = null;
foreach ($cats as $c) { if ($c["code"] === "imzali") { $cat_id_for_imzali = $c["id"]; break; } }
if ($cat_id_for_imzali && !empty($items_by_cat[$cat_id_for_imzali])) {
    render_menu_items($items_by_cat[$cat_id_for_imzali]);
}
?>
      ...
      <section class="tab-panel" id="tab-gurme">
        <h1 class="section-title">Gurme Lezzetler</h1>
        <div class="menu-list">
          
<?php
$cat_id_for_gurme = null;
foreach ($cats as $c) { if ($c["code"] === "gurme") { $cat_id_for_gurme = $c["id"]; break; } }
if ($cat_id_for_gurme && !empty($items_by_cat[$cat_id_for_gurme])) {
    render_menu_items($items_by_cat[$cat_id_for_gurme]);
}
?>
...
@media(max-width:940px){
  ...
  <script>
    ...
    if (initialTab && ['secilmis','imzali','gurme'].includes(initialTab)) {
      activateTab(initialTab);
    } else {
      activateTab('secilmis');
    }
  </script>
