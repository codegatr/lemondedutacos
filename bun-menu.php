...
<head>
  ...
  <title>Le Monde Du Tacos – Bun Menüsü</title>
  ...
  <style>
    ...
    *{ box-sizing:border-box; }
    html,body{ margin:0; padding:0; font-family:system-ui,-apple-system,Segoe UI,Roboto,Arial,sans-serif; color:var(--ink); background:var(--bg); min-height:100vh; display:flex; flex-direction:column; }
    a{ color:inherit; text-decoration:none; }
    button{ font:inherit; }

    /* ── Topbar ── */
    .topbar{ position:sticky; top:0; z-index:999; background:#fff; border-bottom:1px solid rgba(0,0,0,.05); }
    ...
    .nav a.active,.nav a:not(.active):hover{ background:var(--brand); color:#fff; box-shadow:0 6px 18px rgba(58,95,11,.25); }
    .nav a::after{ content:""; position:absolute; top:0; left:-75%; width:50%; height:100%; background:linear-gradient(120deg,transparent 0%,rgba(255,255,255,.6) 50%,transparent 100%); transform:skewX(-20deg); }
    ...
    .menu-shell{ max-width:980px; margin:0 auto; background:#f7f7f7; min-height:calc(100vh - 164px); }

    .menu-head{ position:sticky; top:0; z-index:20; background:#fff; border-bottom:1px solid #ebebeb; box-shadow:0 2px 8px rgba(0,0,0,.06); }

    .searchbar{ display:flex; align-items:center; gap:10px; padding:14px 18px 12px; border-bottom:1px solid #ebebeb; }
    .back-btn,.more-btn{ width:38px; height:38px; border:none; background:transparent; border-radius:50%; display:flex; align-items:center; justify-content:center; cursor:pointer; color:#333; flex:0 0 auto; transition:background .15s; }
    .back-btn:hover,.more-btn:hover{ background:#f0f0f0; }
    .search-input{ flex:1; height:42px; border:1.5px solid #e0e0e0; background:#fafafa; border-radius:8px; display:flex; align-items:center; gap:10px; padding:0 14px; color:#999; min-width:0; }
    ...
    /* ── Tabs ── */
    .tabs{ display:flex; align-items:center; justify-content:center; gap:0; overflow-x:auto; padding:0 18px; background:#fff; scrollbar-width:none; }
    .tabs::-webkit-scrollbar{ display:none; }
    .tab-btn{ position:relative; border:none; background:none; padding:15px 18px 13px; font-size:14px; font-weight:600; color:#8c8c8c; cursor:pointer; white-space:nowrap; flex:0 0 auto; transition:color .15s; }
    .tab-btn.active{ color:#111; }
    .tab-btn.active::after{ content:""; position:absolute; left:0; right:0; bottom:0; height:3px; border-radius:2px 2px 0 0; background:#ff6000; }

    /* ── Section Title ── */
    .section-title{ display:block; font-size:13px; font-weight:600; color:#555; letter-spacing:.2px; padding:16px 16px 10px; border-bottom:1px solid #ebebeb; background:#fff; }

    /* ── Tab panels ── */
    .tab-panel{ display:none; padding:0; background:#f7f7f7; }
    .tab-panel.active{ display:block; }

    /* ── Et Bun — single column list (like image 2) ── */
    .menu-list-single{ display:flex; flex-direction:column; gap:10px; padding:10px; }

    /* ── Tavuk Bun — 2 column grid (like image 3) ── */
    .menu-list{ display:grid; grid-template-columns:1fr 1fr; gap:10px; padding:10px; }

    /* ── Tenders Crousty — single column list (like image 4) ── */
    /* reuses menu-list-single */

    /* ── Product Card ── */
    .menu-item{ display:flex; flex-direction:row; align-items:stretch; background:#fff; border-radius:12px; border:1px solid #ebebeb; overflow:hidden; cursor:pointer; transition:box-shadow .2s,transform .2s; min-height:130px; }
    .menu-item:hover{ box-shadow:0 4px 16px rgba(0,0,0,.10); transform:translateY(-1px); }

    .item-info{ flex:1; padding:14px 14px 14px 12px; display:flex; flex-direction:column; gap:5px; min-width:0; order:2; }
    .item-title{ margin:0; font-size:14px; font-weight:700; line-height:1.25; color:#1a1a1a; }
    .item-price{ font-size:13px; font-weight:600; color:#1a1a1a; display:block; }
    .item-desc{ font-size:12px; line-height:1.5; color:#888; display:-webkit-box; -webkit-line-clamp:3; -webkit-box-orient:vertical; overflow:hidden; margin:0; flex:1; }

    .item-media{ position:relative; width:120px; min-width:120px; background:#e8e8e0; flex-shrink:0; order:1; }
    .item-media img{ width:100%; height:100%; object-fit:cover; display:block; }

    .add-btn{ position:absolute; right:8px; bottom:8px; width:32px; height:32px; border-radius:50%; border:none; background:#3A5F0B; color:#fff; font-size:14px; display:flex; align-items:center; justify-content:center; box-shadow:0 2px 8px rgba(0,0,0,.2); cursor:pointer; transition:background .15s,transform .15s; }
    .add-btn:hover{ background:#2d4a08; transform:scale(1.1); }

    /* ── Promo Bar ── */
    .sticky-promo{ position:sticky; bottom:0; z-index:25; background:#3A5F0B; padding:12px 18px calc(12px + env(safe-area-inset-bottom)); display:flex; align-items:center; gap:12px; box-shadow:0 -2px 12px rgba(0,0,0,.15); }
    .promo-ico{ width:30px; height:30px; border-radius:8px; background:rgba(255,255,255,.2); color:#fff; display:flex; align-items:center; justify-content:center; font-size:14px; flex:0 0 auto; }
    .promo-text{ font-size:13px; font-weight:700; color:#fff; }

    /* ── Footer ── */
    .footer{ max-width:var(--max); margin:0 auto; padding:18px 18px 26px; display:flex; align-items:center; justify-content:space-between; gap:14px; color:var(--muted); font-size:12px; width:100%; background:#fff; }
    .social-nav{ padding:0; margin:0; list-style:none; display:flex; align-items:center; gap:10px; }
    .social-nav li{ display:inline-block; }
    .social-nav a{ display:inline-block; width:36px; height:36px; line-height:36px; text-align:center; color:#fff; text-decoration:none; background:#000; border-radius:8px; position:relative; transition:.35s ease; overflow:hidden; font-size:18px; }
    .model-2 a{ overflow:hidden; font-size:20px; border-radius:10px; margin:0; }
    .model-2 a:hover{ background:#fff; text-shadow:0px 0px #d5d5d5,1px 1px #d5d5d5,2px 2px #d5d5d5,3px 3px #d5d5d5,4px 4px #d5d5d5,5px 5px #d5d5d5,6px 6px #d5d5d5,7px 7px #d5d5d5,8px 8px #d5d5d5,9px 9px #d5d5d5,10px 10px #d5d5d5; }
    ...
    @media (max-width:940px){
      ...
      .nav{ position:absolute; top:58px; right:12px; left:12px; background:#fff; border:1px solid rgba(0,0,0,.10); border-radius:14px; box-shadow:0 10px 30px rgba(0,0,0,.18); padding:10px; display:none; flex-direction:column; align-items:stretch; gap:6px; z-index:9998; }
      ...
      .footer{ flex-shrink:0; width:100%; margin:0; padding:6px 14px 8px; font-size:10px; gap:8px; }
      .social-nav{ gap:6px; }
      .social-nav a,.model-2 a{ width:28px; height:28px; line-height:28px; font-size:14px; border-radius:6px; }
    }
...
<label class="search-input" for="menuSearch">
  ...
  >
        ...
        <div class="tabs" role="tablist" aria-label="Bun kategorileri">
          <button class="tab-btn active" role="tab" aria-selected="true" data-tab="et-bun">Et Bun</button>
          <button class="tab-btn" role="tab" aria-selected="false" data-tab="tavuk-bun">Tavuk Bun</button>
          <button class="tab-btn" role="tab" aria-selected="false" data-tab="tenders-crousty">Tenders Crousty</button>
        </div>
      </div>

      <!-- ET BUN TAB — single column (görsel 2 stili) -->
      <section class="tab-panel active" id="tab-et-bun">
        <h1 class="section-title">Et Bun</h1>
        <div class="menu-list-single">

          
<?php
$cat_id_for_et_bun = null;
foreach ($cats as $c) { if ($c["code"] === "et-bun") { $cat_id_for_et_bun = $c["id"]; break; } }
if ($cat_id_for_et_bun && !empty($items_by_cat[$cat_id_for_et_bun])) {
    render_menu_items($items_by_cat[$cat_id_for_et_bun]);
}
?>
      ...
      <!-- TAVUK BUN TAB — 2 column grid (görsel 3 stili) -->
      <section class="tab-panel" id="tab-tavuk-bun">
        <h1 class="section-title">Tavuk Bun</h1>
        <div class="menu-list">

          
<?php
$cat_id_for_tavuk_bun = null;
foreach ($cats as $c) { if ($c["code"] === "tavuk-bun") { $cat_id_for_tavuk_bun = $c["id"]; break; } }
if ($cat_id_for_tavuk_bun && !empty($items_by_cat[$cat_id_for_tavuk_bun])) {
    render_menu_items($items_by_cat[$cat_id_for_tavuk_bun]);
}
?>
      ...
      <!-- TENDERS CROUSTY TAB — single column (görsel 4 stili) -->
      <section class="tab-panel" id="tab-tenders-crousty">
        <h1 class="section-title">Tenders Crousty</h1>
        <div class="menu-list-single">

          
<?php
$cat_id_for_tenders_crousty = null;
foreach ($cats as $c) { if ($c["code"] === "tenders-crousty") { $cat_id_for_tenders_crousty = $c["id"]; break; } }
if ($cat_id_for_tenders_crousty && !empty($items_by_cat[$cat_id_for_tenders_crousty])) {
    render_menu_items($items_by_cat[$cat_id_for_tenders_crousty]);
}
?>
...
<script>
  ...
  const VALID_TABS = ['et-bun', 'tavuk-bun', 'tenders-crousty'];
  let selectedTab = 'et-bun';
  ...
  function updatePanelSections(panel) {
    ...
    const list = panel.querySelector('.menu-list, .menu-list-single');
  ...
  if (initialTab && VALID_TABS.includes(initialTab)) {
    ...
    activateTab('et-bun', { remember: true });
  }
